...
from sqlalchemy.orm import column_property, declared_attr, object_session, relationship
        ...
        for module_dist in iter_modules_dist():
            ...
            if is_current_module_installed:
                ...
                if module_statistics:
                    nb_observations = module_statistics.get_dataset_nb_observations(
                        self.id_dataset
                    )
                    request_nb_observations += nb_observations

        return request_nb_observations

    @property
    def nb_observations_habitats(self):
        from geonature.utils.module import is_module_installed, iter_modules_dist

        for module_dist in iter_modules_dist():
            module_name = module_dist.name
            if module_name == "gn_module_occhab":
                is_current_module_installed = is_module_installed(module_name)
                if is_current_module_installed:
                    module_statistics = None
                    try:
                        ...
                        pass
                    if module_statistics:
                        request_nb_observations = module_statistics.get_dataset_nb_observations(
                            self.id_dataset
                        )

        return object_session(self).scalar(request_nb_observations)
